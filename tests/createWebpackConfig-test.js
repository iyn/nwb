...
describe('createWebpackConfig()', () => {
  context('with only entry config', () => {
    ...
    it('creates a default webpack build config', () => {
      ...
      expect(config.module.rules.map(rule => rule.loader).join('\n'))
  ...
  context('with server config', () => {
    ...
    it('creates a server webpack config', () => {
      expect(config.module.rules.map(rule => {
        // The style pipeline will have a list of chained loaders
        if (Array.isArray(rule.loader)) return rule.loader.map(loader => loader.loader).join('\n')
        return rule.loader
      }).join('\n'))
        .toContain('babel-loader')
...
describe('mergeRuleConfig()', () => {
  ...
  it('appends lists when merging options', () => {
    expect(mergeRuleConfig(
      rule,
